...
//                Log.d(TAG, "onMessageReceived mGroupChannel - url- ${mGroupChannel?.url}")
    ...
    private fun setOnclickListeners(vararg views: View) {
        for (view in views) {
            ...
            view.setOnClickListener { p0 ->
                when (p0?.id) {
                    img_flower.id -> {
                        initiateMessage(text_other_user.text.toString(), "flower")
                    }
                    img_home.id -> {
                        initiateMessage(text_other_user.text.toString(), "home")
                    }
                    img_sofa.id -> {
                        initiateMessage(text_other_user.text.toString(), "sofa")
                    }
                }
            }
        }
    ...
    private fun initiateMessage(userId: String, customType: String) {
